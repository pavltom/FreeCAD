    ...
    , myName(name)
{
    ...
    connect(MainWindow::getInstance(), &QMainWindow::tabifiedDockWidgetActivated, this, &TreeWidget::onTabifiedDockWidgetActivated);
...
int DocumentItem::findRootIndex(const ViewProviderDocumentObject *childObj) const {
    if (!TreeParams::getKeepRootOrder() || !childObj || !childObj->getObject()
